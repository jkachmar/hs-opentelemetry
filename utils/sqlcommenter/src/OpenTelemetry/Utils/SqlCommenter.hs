...
{- |
Module      : OpenTelemetry.Utils.SqlCommenter
...
(via the thread-local context). Returns an empty map if no attributes are found.

Attributes are stored under a dedicated context key; populate them by inserting
a @'Map' 'Text' 'Text'@ into the active context before calling this.
-}
...
The @traceparent@ and @tracestate@ values are produced by encoding the span's
'Core.SpanContext' according to the W3C Trace Context format, and are inserted
into the supplied attribute map under the @"traceparent"@ and @"tracestate"@ keys.
-}
...
If the active context contains a span, its @traceparent@ and @tracestate@ are added
to the returned attributes; otherwise the stored attributes are returned unchanged.
-}
...
-}
urlEncodeBuilder' :: Word8Set -> Text -> StrictTextBuilder
...
urlEncodeBuilder :: Text -> StrictTextBuilder
