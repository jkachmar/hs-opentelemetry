...
import OpenTelemetry.Utils.SqlCommenter
import qualified OpenTelemetry.Utils.SqlCommenter as SqlCommenter
import Test.Hspec
...
main = hspec $ do
  ...
  specify "Span attributes are picked up from thread-local context" $ do
    -- A processor is required so that 'inSpan' creates a real (recording) span
    -- and inserts it into the thread-local context. With no processors, the SDK
    -- takes a fast path that drops the span and skips context modification.
